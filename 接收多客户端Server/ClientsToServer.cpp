...
BOOL _stdcall _ToClient(Screen* lpScreen)
{
	...
	while (1)
	{
		...
		if (flag == 1)		//如果recv出错,断开连接结束线程
		{
			...
			ListView_DeleteItem(GetDlgItem(hWinMain, IDC_LIST1), pClient->stScreen.dwItem);		//删除对应UI中的连接
			break;																				//退出线程
		}
...
BOOL CALLBACK _DialogScreen(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		pClient->stScreen.hWnd = hWnd;
		pClient->stScreen.dwItem = lParam;

		pClient->hScreenThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)_ShowScreen, &pClient->stScreen, 0, NULL);
